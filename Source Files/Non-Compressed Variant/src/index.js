...
async function compression(image) {
  ...
  //let width = (image.width-512)/1536
  //let height = (image.height-510)/1024
  ...
  var oldSize = image.src.split("data:image/jpeg;base64,").join("")
  //console.log(oldSize)
  ...
  await model.fitDataset(ds,
    {
      ...
      callbacks: {
        onEpochEnd: async (epoch, logs) => {
          ...
          lossOutput.innerHTML = 'Loss: ' + logs.loss + " Accuracy: " + logs.acc
          // await tf.nextFrame();
...
async function functionalTraining() {
  ...
  let output = model.evaluate([testDataInputTensorWidth, testDataInputTensorWHeight], testDataOutputTensor,  {Sizeofbatch: 3})
